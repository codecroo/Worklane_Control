        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    exit="exit"
                >
                    <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-white transition" aria-label="Close modal">
                        <X />
                    </button>

                    <h2 className="text-xl font-semibold mb-6">{editingProject ? "Edit Project" : "Add Project"}</h2>
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            animate={{
                                                ...
                                                boxShadow: isSelected ? "0 0 100px 15px rgba(110, 110, 110, 0.45)" : "none",
                                            }}
                        ...
                        <motion.div className="relative w-full" custom={3} initial="hidden" animate="visible" variants={inputVariants}>
                            ...
                            <div className="mt-2 flex flex-col gap-2">
                                {formData.tasks.map((task, idx) => (
                                    <div key={idx} className="flex items-center justify-between bg-blue-500/10 text-white border border-blue-300/30 rounded-md px-3 py-2 text-sm select-none">
                                        <div className="flex items-center gap-2">
                                            ...
                                            />
                                            <span className={task.is_completed ? "line-through text-gray-400" : ""}>{task.name}</span>
                                        </div>
                                        <button type="button" onClick={() => removeTask(task.name)} className="hover:text-indigo-300 transition" aria-label={`Remove task ${task.name}`}>
                                            <X size={14} />
                                        </button>
                                    </div>
                                ))}
